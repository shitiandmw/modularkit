...
export class Loader {
    ...
    static LoadServices(servicePath: string, app: any, ctx?: any) {
        return this.loadClass(servicePath, app, ctx);
    }

    static LoadControllers(controllerPath: string, app: any, servicePath?:string) {
                            ...
                            get: (proto, methodName) => {
                                if (typeof proto[methodName] === 'function') {
                                    return async (ctx: any) => {
                                        // 加载服务代理
                                        if(servicePath) ctx.service = this.LoadServices(servicePath, app, ctx);
                                        // 设置请求上下文
    ...
    static LoadModels(modelPath: string, mongoInterface: any) {
        return new Proxy({}, {
            get: (instances: any, name: string) => {
                if (!instances[name]) {
